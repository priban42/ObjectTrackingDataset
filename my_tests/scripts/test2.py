...
def derivative(values, times, index):
    ...
    if index < width:
        ...
        return (((values[min(index + width, max_index)] - values[index]) / (
                    times[min(index + width, max_index)] - times[index]))
...
def arr_filter(values, times):
    ret = []
    for i in range(len(values)):
        ret.append(moving_average(values, times, i, 5, 200))
    return ret
...
def filter_joint_space(values, times):
    ret = []
    for joint in values:
        ret.append(arr_filter(joint, times))
    return ret
        ...
        for point in plan.joint_trajectory.points:
            reduced_plan.append(
                {"positions": point.positions, "velocities": point.velocities, "accelerations": point.accelerations,
                 "nsecs": point.time_from_start.nsecs, "secs": point.time_from_start.secs})
        PATH = "/home/bagr/PycharmProjects/ObjectTrackingDataset/dataset/trajectory_" + str(NUMBER)
...
def main():
    try:
        ...
        plan_post_process(cartesian_plan, 1)

        tutorial.export_plan(cartesian_plan, 0)
        tutorial.export_semi_reduced_plan(cartesian_plan, 0)
