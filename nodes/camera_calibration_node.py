    ...
    for pose_nb, line in enumerate(pos_to_take):
        ...
        if not 'joint' in positions_file:
            ...
            try:
                ...
                arm.go(wait=True)
                rospy.loginfo('Pre-pose reached.')
                # joint_goal = arm.get_current_joint_values() #  dirty hack!
                # joint_goal[6] = joint_goal[6] - pi/4 #  dirty hack!
                # arm.go(joint_goal, wait=True) #  dirty hack!
                # arm.stop() #  dirty hack!
                rospy.loginfo('Rotated-pose reached.')
                pose_count += 1
            except ValueError as e:
                rospy.logfatal("CALIBRATION: " + e)
                rospy.logfatal("CALIBRATION: Exiting!")
                ...
                rospy.logfatal(f'Cannot reach the position. Error: {e}. Going to the next pose.')
                continue
        else:
        ...
        print(f'image time: {last_image.header.stamp}')
        if use_time:
